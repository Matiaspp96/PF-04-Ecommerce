...
export default function Card({ producto, quantity, cart, setCart }) {
	...
	return (
		<Flex
			...
			h={{
				base:
					router.route === '/cart' || router.route === '/favorites'
						? '58vh'
						: '46vh',
				md: '290',
				lg: router.route === '/cart' ? '430' : '365',
			}}
			w={{
				base:
					router.route === '/cart' || router.route === '/favorites'
		...
		>
            ...
            zIndex='2' key={Math.random()} minW='fit-content' minH='fit-content' overflow='hidden' borderRadius='20px'>> */}
					<Image
						...
						width={router.route === '/cart' ? '250' : '190'}
						height={router.route === '/cart' ? '250' : '190'}
                ...
                <Button alignItems='center' w='2rem' h='2rem' display='flex' onClick={e=>handleAddCartOnClick(e,product)}>+</Button> */}
									...
									<NumberInput min={1} max={stock}>
										...
										<NumberInputStepper>
											<NumberIncrementStepper
												...
												onClick={e => {
													if (quantity < stock) {
														handleAddCartOnClick(e, product);
													}
												}}
											/>
											<NumberDecrementStepper
												onClick={e => handleRemoveCartOnClick(e, product)}
											/>
										</NumberInputStepper>
									</NumberInput>
								</InputGroup>
						...
						<Flex alignItems={'center'}>
							<Text fontWeight={300} fontSize='md' me={'.4em'}>
								{rating}
							</Text>
					...
					{cart ? (
						...
						>
							...
							/>
							{quantity === stock ? (
								<Text color={'#1884BE'}>{quantity} Max</Text>
							) : (
						...
						<Flex justifyContent={'space-evenly'} width='100%'>
							<IconButton
								...
								aria-label='add to cart'
								icon={
									addCart ? <IoCart size='2em' /> : <IoCartOutline size='2em' />
								}
								color={addCart ? '#1884BE' : 'grey'}
							/>
