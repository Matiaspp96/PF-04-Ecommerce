...
import {
	addItemToFav,
	deleteItemOfFav,
} from '../../redux/actions/favorites.js';
...
export default function Detail() {
	...
	const getSugestions = (arr, cat) => {
		const ps = arr.filter(pr => {
			if (pr.category.includes(cat)) {
				return pr;
			}
		});
		return ps.slice(0, 3);
						...
						<Stack spacing={{ base: 6, md: 10 }}>
							...
							>
								{product.category?.map(e => (
									<Text key={e}>
										{getCategoryName(e)}{' '}
										{e !== product.category[product.category.length - 1]
											? '-'
											: ''}
									</Text>
								))}
							...
							<Flex>
								<Text fontWeight={'medium'} fontSize={'2xl'} color={'#1884BE'}>
									$ {product.price}
								</Text>
								<Flex ml={'3rem'} alignItems={'center'}>
									<Text fontWeight={300} fontSize={'2xl'} me={'.5em'}>
										{product.rating}
									</Text>
							...
							<Tabs variant='enclosed'>
								...
								<TabPanels>
									<TabPanel>
										...
										>
											<Box>
												<Text padding='2' fontSize={'larger'}>
													{product.description}
												</Text>
											</Box>
										</Flex>
									...
									<TabPanel>
										<Review
											reviews={product.reviews.map(rs => {
												return rs._id;
											})}
											id={id}
											postReview={setNewReviewAdded}
										/>
									</TabPanel>
								</TabPanels>
							</Tabs>
						</Container>
						<Container overflow={{ base: 'visible', lg: 'auto' }}>
							<Center fontSize='3xl' fontWeight={'bold'}>
								You may also like
							</Center>
							{getSugestions(sugestions, product.category[0]).map(ps => {
								return <CardMinimal key={ps._id} producto={ps} />;
							})}
						</Container>
					</SimpleGrid>
				</>
