...
import { AiOutlineBorderlessTable , AiOutlinePhone } from 'react-icons/ai';
...
const CheckoutForm = () => {
	...
	const itemsMP = itemsCart?.map(item =>
		typeof item === 'object'
			? {
              ...
              <FormControl isRequired>
                  ...
                  <Input id='lastname' name='last_name' value={last_name} onChange={e => handleChange(e)} type='text' autoComplete='off'/>
					...
					<FormControl isRequired>
						...
						{error.state ? (
							<FormHelperText color='red.400'>{error.state}</FormHelperText>
						) : null}
					</FormControl>
					<FormControl isRequired>
						...
						<InputGroup>
							<InputLeftElement pointerEvents='none'>
								<IoLocationOutline color='gray.300' />
							</InputLeftElement>
							<Input
								placeholder='Address...'
								id='address'
								name='street'
								value={buyer.shipping.street}
								onChange={e => handleShipping(e)}
								type='text'
								autoComplete='off'
							/>
						...
						{error.street ? (
							<FormHelperText color='red.400'>{error.street}</FormHelperText>
						) : (
							<FormHelperText>
					...
					<FormControl>
						...
						<InputGroup>
							<InputLeftElement pointerEvents='none'>
								<AiOutlineBorderlessTable color='gray.300' />
							</InputLeftElement>
							<Input
								placeholder='Between...'
								name='between'
								value={buyer.shipping.between}
								onChange={e => handleShipping(e)}
								type='text'
								autoComplete='off'
							/>
					...
					<FormControl>
						...
						<InputGroup>
							<InputLeftElement pointerEvents='none'>
								<MdOutlineApartment color='gray.300' />
							</InputLeftElement>
							<Input
								id='apartment'
								name='floor'
								value={buyer.shipping.floor}
								onChange={e => handleShipping(e)}
								placeholder='Floor...'
								type='text'
								autoComplete='off'
					...
					<FormControl isRequired>
						...
						{error.zip ? (
							<FormHelperText color='red.400'>{error.zip}</FormHelperText>
						) : null}
					</FormControl>
					<FormControl isRequired>
				...
				>
					<Center>
						<Heading as='h4' size='md'>
							Your Order: {itemsCart.length} Items
						</Heading>
					</Center>
					...
					<InputGroup flexDir='column' flexWrap='wrap'>
						...
						<Flex flexDir='row'>
							<Input
								w={{ base: '90%', md: '90%' }}
								bgColor={colorMode === 'light' ? 'whiteAlpha.800' : 'gray.700'}
								placeholder='Card...'
							></Input>
							<Button
								border='1px solid #348099'
								_hover={{ bgColor: '#348099' }}
							>
								<IoGiftOutline />
							</Button>
						</Flex>
