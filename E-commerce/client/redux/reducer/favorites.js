...
export default function favoritesReducer(state = initialState, action) {
	const itemsFav = state.itemsFav;
	let favs = state.itemsFav;
	switch (action.type) {
		case ADD_ITEM_FAVORITES:
			const itemFav = itemsFav?.find(e => e._id === action.payload._id);
			if (itemFav) {
				favs = itemsFav.map(e => {
		...
		case DELETE_ITEM_FAVORITES:
			const index = itemsFav.findIndex(e => e._id === action.payload);
			const newFav = [...itemsFav];
			if (index >= 0) {
