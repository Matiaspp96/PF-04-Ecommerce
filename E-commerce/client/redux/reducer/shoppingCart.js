...
export default function shoppingCartReducer(state = initialState, action) {
	...
	switch (action.type) {
		case ADD_ITEM:
			...
			if (itemCart) {
				cart = state.itemsCart.map(e =>
					e.product._id === action.payload._id
						? {
								...e,
								quantity: e.quantity + 1,
								totalPrice: action.payload.price + e.quantity * e.product.price,
						  }
						: e
				);
			} else {
				...
				cart = state.itemsCart.map(e =>
					e.product._id === action.payload._id
						? {
								...
								totalPrice: action.payload.quantity * e.product.price,
						  }
						: e
				);
			}
		...
		case REMOVE_ITEM: // Remover cantidades del articulo
			...
			if (index >= 0 && itemsCart[index].quantity > 1) {
				cart = itemsCart.map(e =>
					e.product._id === action.payload.id
						? {
								...e,
								quantity: e.quantity - 1,
								totalPrice:
									state.totalPrice + (e.quantity - 1) * e.product.price,
						  }
						: e
				);
			}
